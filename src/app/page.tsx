"use client";

import { useState } from "react";
import { ProgressProvider, useProgress } from "@/lib/use-progress";
import { MODULE_META, ACCENTS } from "@/components/learn/accents";
import { Module01Terms } from "@/components/learn/module-01-terms";
import { Module02Weights } from "@/components/learn/module-02-weights";
import { Module03Error } from "@/components/learn/module-03-error";
import { Module04Training } from "@/components/learn/module-04-training";
import { Module05Data } from "@/components/learn/module-05-data";
import { Module06Future } from "@/components/learn/module-06-future";
import { Module07Cycle } from "@/components/learn/module-07-cycle";
import { Module08Spam } from "@/components/learn/module-08-spam";
import { Module09Explain } from "@/components/learn/module-09-explain";
import { Module10Next } from "@/components/learn/module-10-next";
import { Pitfalls } from "@/components/learn/pitfalls";
import { Resources } from "@/components/learn/resources";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Progress } from "@/components/ui/progress";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import {
  Brain,
  Sparkles,
  ArrowDown,
  Github,
  Heart,
  Trash2,
  CheckCircle2,
  FlaskConical,
} from "lucide-react";

function Hero() {
  const { completedCount, totalCount, hydrated, resetAll } = useProgress();
  const progressPct = hydrated ? (completedCount / totalCount) * 100 : 0;

  return (
    <section className="relative overflow-hidden border-b">
      {/* Декоративный фон (приглушается в тёмной теме через .hero-decoration) */}
      <div className="hero-decoration absolute inset-0 opacity-[0.04] pointer-events-none">
        <div className="absolute top-10 left-10 text-[200px] font-bold font-mono text-teal-900 dark:text-teal-400">
          Σ
        </div>
        <div className="absolute bottom-10 right-10 text-[150px] font-bold font-mono text-amber-900 dark:text-amber-400">
          w·x
        </div>
        <div className="absolute top-1/2 left-1/3 text-[120px] font-bold font-mono text-rose-900 dark:text-rose-400">
          ∇
        </div>
      </div>

      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="max-w-2xl">
            <Badge
              variant="outline"
              className="mb-3 bg-teal-50 border-teal-200 text-teal-700 dark:bg-teal-950/50 dark:border-teal-800/60 dark:text-teal-300"
            >
              <Sparkles className="h-3 w-3 mr-1" />
              Интерактивный курс для новичков
            </Badge>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight leading-tight">
              Веса и обучение модели
            </h1>
            <p className="mt-4 text-base sm:text-lg text-muted-foreground leading-relaxed">
              10 модулей с живыми песочницами на JavaScript. Покрути признаки,
              подгони веса, посмотри, как модель учится на ошибке. Никакой
              магии — только числа, которые меняются.
            </p>

            <div className="flex flex-wrap gap-3 mt-6">
              <a href="#module-1">
                <Button
                  size="lg"
                  className="bg-teal-600 hover:bg-teal-700 text-white dark:bg-teal-700 dark:hover:bg-teal-600"
                >
                  Начать с первого модуля
                  <ArrowDown className="h-4 w-4 ml-1.5" />
                </Button>
              </a>
            </div>
          </div>

          {/* Карточка прогресса */}
          <Card className="p-4 w-full sm:w-64 bg-card/95">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs uppercase tracking-wide text-muted-foreground font-semibold">
                Твой прогресс
              </span>
              {hydrated && completedCount === totalCount && (
                <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
              )}
            </div>
            <div className="text-2xl font-bold font-mono">
              {hydrated ? completedCount : 0}
              <span className="text-base text-muted-foreground font-normal">
                {" "}
                / {totalCount}
              </span>
            </div>
            <Progress
              value={progressPct}
              className="h-2 mt-2"
            />
            <p className="text-xs text-muted-foreground mt-2">
              модулей отмечено как пройденные
            </p>
            {hydrated && completedCount > 0 && (
              <Button
                type="button"
                variant="ghost"
                size="sm"
                className="w-full mt-2 h-7 text-xs text-muted-foreground"
                onClick={resetAll}
              >
                <Trash2 className="h-3 w-3 mr-1" />
                Сбросить прогресс
              </Button>
            )}
          </Card>
        </div>

        {/* Бейджи характеристик */}
        <div className="flex flex-wrap gap-2 mt-8">
          <Badge variant="secondary" className="bg-amber-50 border-amber-200 text-amber-800 dark:bg-amber-950/50 dark:border-amber-800/60 dark:text-amber-300">
            <FlaskConical className="h-3 w-3 mr-1" />
            10 живых песочниц
          </Badge>
          <Badge variant="secondary" className="bg-rose-50 border-rose-200 text-rose-800 dark:bg-rose-950/50 dark:border-rose-800/60 dark:text-rose-300">
            <Brain className="h-3 w-3 mr-1" />
            Без формул — сначала смысл
          </Badge>
          <Badge variant="secondary" className="bg-violet-50 border-violet-200 text-violet-800 dark:bg-violet-950/50 dark:border-violet-800/60 dark:text-violet-300">
            32 бесплатных ресурса
          </Badge>
          <Badge variant="secondary" className="bg-emerald-50 border-emerald-200 text-emerald-800 dark:bg-emerald-950/50 dark:border-emerald-800/60 dark:text-emerald-300">
            Прогресс сохраняется
          </Badge>
        </div>
      </div>
    </section>
  );
}

function ModuleNav() {
  const { isCompleted, hydrated } = useProgress();
  return (
    <nav
      className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b"
      aria-label="Навигация по модулям"
    >
      <div className="max-w-6xl mx-auto px-2 sm:px-6">
        <div className="flex items-center gap-1 overflow-x-auto py-2 scrollbar-thin">
          {MODULE_META.map((m) => {
            const done = hydrated && isCompleted(m.id);
            const accent = ACCENTS[m.id];
            return (
              <a
                key={m.id}
                href={`#module-${m.id}`}
                className={cn(
                  "shrink-0 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors border",
                  done
                    ? "bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-950/50 dark:border-emerald-800/60 dark:text-emerald-300"
                    : cn("border-transparent hover:bg-muted", accent.text)
                )}
              >
                <span className="font-mono mr-1">{m.id}.</span>
                {m.short}
                {done && <CheckCircle2 className="inline h-3 w-3 ml-1" />}
              </a>
            );
          })}
          <a
            href="#pitfalls"
            className="shrink-0 px-2.5 py-1.5 rounded-md text-xs font-medium text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-950/40"
          >
            Ошибки
          </a>
          <a
            href="#resources"
            className="shrink-0 px-2.5 py-1.5 rounded-md text-xs font-medium text-teal-700 dark:text-teal-300 hover:bg-teal-50 dark:hover:bg-teal-950/40"
          >
            Ресурсы
          </a>
        </div>
      </div>
    </nav>
  );
}

function MainContent() {
  return (
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      <Module01Terms />
      <Module02Weights />
      <Module03Error />
      <Module04Training />
      <Module05Data />
      <Module06Future />
      <Module07Cycle />
      <Module08Spam />
      <Module09Explain />
      <Module10Next />
      <Pitfalls />
      <Resources />
    </main>
  );
}

function SiteFooter() {
  return (
    <footer className="mt-auto border-t bg-muted/30">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 text-sm text-muted-foreground">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Brain className="h-4 w-4 text-teal-600 dark:text-teal-400" />
            <span>
              <strong className="text-foreground">ML с нуля</strong> —
              интерактивный курс про веса и обучение модели
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-xs">
            Сделано с <Heart className="h-3 w-3 fill-rose-500 text-rose-500 dark:fill-rose-400 dark:text-rose-400" /> для
            новичков в машинном обучении
          </div>
        </div>
        <p className="text-xs mt-3 max-w-3xl">
          Все песочницы работают прямо в браузере на чистом JavaScript (React + TypeScript).
          Прогресс сохраняется локально в localStorage — твои ответы и метки
          не уходят на сервер. Кураторские ссылки на бесплатные ресурсы
          собраны из живого веб-поиска и проверены на актуальность.
        </p>

        {/* Подпись создателя — по центру, отдельной строкой */}
        <div className="mt-6 pt-4 border-t border-border/60 text-center">
          <span className="text-sm font-medium text-muted-foreground">
            создатель{" "}
            <span className="font-bold tracking-wide text-foreground">AZAR</span>
          </span>
        </div>
      </div>
    </footer>
  );
}

export default function Home() {
  return (
    <ProgressProvider>
      <div className="min-h-screen flex flex-col bg-background">
        <Hero />
        <ModuleNav />
        <MainContent />
        <SiteFooter />
      </div>
    </ProgressProvider>
  );
}
